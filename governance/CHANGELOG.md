...
- `[M365]` ⏳ **Pendente confirmar:** se "SMTP autenticado" (Authenticated SMTP) está habilitado para `joao@jpxdigital.com.br` — necessário para migrar o envio do Zoho pro M365. Verificado que o servidor `smtp.office365.com:587` anuncia `AUTH LOGIN XOAUTH2` (não bloqueado a nível de tenant), mas isso não confirma o toggle específico da caixa — investigação parada no meio (painel do Microsoft 365 Admin Center → Usuários ativos → conta → aba Email → "Gerenciar aplicativos de e-mail", procurando o item específico "SMTP autenticado")
